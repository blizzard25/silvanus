...
contract SVNPresale is Ownable, ReentrancyGuard {
    ...
    function buyTokens() external payable nonReentrant {
        ...
        (tokensToBuy, cost) = _calculateTokenAmount(msg.value);
    ...
    function _calculateTokenAmount(uint256 ethAmount) internal view returns (uint256 tokens, uint256 cost) {
        ...
        if (remainingTier1 > 0) {
            ...
            uint256 purchasable = (remaining * 1e18) / tierPrice;
            uint256 actual = purchasable > remainingTier1 ? remainingTier1 : purchasable;
            uint256 tierCost = (actual * tierPrice) / 1e18;
            tokens += actual;
            cost += tierCost;
            remaining -= tierCost;
            remainingTier1 -= actual;
        }
